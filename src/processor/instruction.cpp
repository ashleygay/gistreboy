...
#define LD_RegX_def(reg)\
	...
	{\
	uint8_t val = boost::get<uint8_t>(this->_arg[0]);\
	p->reg.value = val;\
	}
...
{
	uint16_t address = make_word(p->L.value, p->H.value);
	uint8_t value = boost::get<uint8_t>(this->_arg[0]);
	p->_write(value, address);
}
...
{
	uint16_t address = boost::get<uint16_t>(this->_arg[0]);
...
{
	uint16_t address = 0xFF00 + boost::get<uint8_t>(this->_arg[0]);
	p->_write(p->A.value, address);
}
...
{
	uint16_t BC = boost::get<uint16_t>(this->_arg[0]);
...
{
	uint16_t DE = boost::get<uint16_t>(this->_arg[0]);
...
{
	uint16_t HL = boost::get<uint16_t>(this->_arg[0]);
	p->L.value = get_low(HL);
	p->H.value = get_high(HL);
}
...
{
	p->SP.value = boost::get<uint16_t>(this->_arg[0]);
}
...
{
	uint8_t n = boost::get<uint8_t>(this->_arg[0]);
...
{
  auto tmp = boost::get<uint8_t>(this->_arg[0]);
  uint val = p->A.value + tmp;
  uint8_t result = static_cast<uint8_t>(val);
  p->flag.unsetFlag(FlagRegister::ZERO);
  p->flag.unsetFlag(FlagRegister::HALFCARRY);
...
{
	uint8_t byte = boost::get<uint8_t>(this->_arg[0]);
...
{
	p->PC.value = boost::get<uint16_t>(this->_arg[0]);
}
...
{
	p->PC.value += (int8_t) boost::get<uint8_t>(this->_arg[0]);
}
...
{
	if ( p->flag.getFlag(FlagRegister::ZERO)) {
		int8_t t = (int8_t) boost::get<uint8_t>(this->_arg[0]);
		p->PC.value += t;
	}
}
