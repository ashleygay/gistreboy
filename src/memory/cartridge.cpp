...
{
	if (range(address, 0x00, 0x3FFF))
	{
		if (address == 0x100)
			has_boot = true;
		if (range(address, 0x00, 0xFF) & !has_boot)
			return boot_rom[address];
		else
			return rom[address];
	}

	else if (range(address, 0x4000, 0x7FFF))
	{
		if (mbc = 0)
			return rom[adress];
		else
	...
	else if (range(address, 0xA000, 0xBFFF))
...
{
	if (range(address, 0xA000, 0xBFFF))
		ram[address + get_current_ram_bank()*8192 - 0xA000] = byte;

	else if (range(address, 0x00, 0X1FFF))
		ram_enable = (byte & 0x0F == 0x0A)

	else if (range(address, 0x2000, 0x3FFF))
	{
		if (byte > 0x1F)
	...
	else if (range(address, 0x4000, 0x5FFF))
		ram_bank_number = byte;
	else if (range(address, 0x6000, 0x7FFF))
	{
		if (byte == 0x00)
			rom_ram_mode = false;
		else if (byte == 0x01)
...
{
	if (rom_ram_mode)
		return (0 << 3)+(rom_bank_number << 5);
	else
		return (0 << 1)+(ram_bank_number << 2)+(rom_bank_number << 5);
